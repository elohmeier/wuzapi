...
func main() {
	...
	dbDirectory := cwdPath + "/dbdata"
	_, err = os.Stat(dbDirectory)
	if os.IsNotExist(err) {
		errDir := os.MkdirAll(dbDirectory, 0751)
		if errDir != nil {
			panic("Could not create dbdata directory: " + dbDirectory)
		}
	}

	db, err := sql.Open("sqlite", cwdPath+"/dbdata/users.db")
	if err != nil {
		log.Fatal().Err(err).Msg("Could not open/create " + cwdPath + "/dbdata/users.db")
		os.Exit(1)
	}
	defer db.Close()
